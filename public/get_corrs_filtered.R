...
## This version assumes we're providing a list of cancer types of interest
...
#species <- read.table('Species_list.txt')[,1] # One species per line. See example file
...
## Load the list of cancer types
cancers <- read.table('C:/Users/gi.padovani/OneDrive - University of Florida/MammalianCancer/PhyloP_PanMammalian/PhyloP_PanMammalian/cancer_types_filtered.txt',sep='\t')[,1] # One species per line. See example file
...
for(i in unique(species)) {
  ...
  for(j in cancers) {
    genes <- which(str_detect(dat$cancer_type, j))
...
write.table(corrs_species, file='Species_correlations_filt_g.txt')
...
write.table(top_species, file='Top_species_correlations_filt_g.txt')
